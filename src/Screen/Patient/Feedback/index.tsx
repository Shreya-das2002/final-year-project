            ...
            <tr className="bg-gray-50">
              ...
              {ratingColumns.map((column) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-500">{column.value}</div>
                </th>
              ))}
            </tr>
          </thead>
...
const Feedback: React.FC = () => {
  ...
  const handleConsultationChange = (value: boolean) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!patientId) {
        const message = "Patient information was not found. Please log in again.";
      ...
      if (!overallRating) {
        const message = "Please select your overall experience rating.";
        setSubmitError(message);
        toast.error(message);
        return;
      }

      const aiAccuracy = areaRatings["AI Symptom Checker Accuracy"];
      const websiteRating = areaRatings["Website Design & UI"];

      if (!aiAccuracy) {
        const message = "Please rate AI Symptom Checker Accuracy.";
        setSubmitError(message);
        toast.error(message);
        return;
      }
      ...
      if (!websiteRating) {
        const message = "Please rate Website Design & UI.";
        setSubmitError(message);
        toast.error(message);
        return;
      }
      ...
      if (consultedDoctor) {
        if (!selectedAppointment) {
          const message = "Please select an appointment for feedback.";
          setSubmitError(message);
          toast.error(message);
          return;
        }

        const missingConsultationRating = consultationRatingAreas.find(
          (area) => !areaRatings[area]
        );

        if (missingConsultationRating) {
          const message = `Please rate ${missingConsultationRating}.`;
          setSubmitError(message);
          toast.error(message);
          return;
        }
      ...
      const payload: CreatePatientFeedbackPayload = {
        ...
        appointment_id:
          consultedDoctor && selectedAppointment
            ? Number(selectedAppointment.appointment_id)
            : null,

        experience: overallRating,

        booking: consultedDoctor ? areaRatings["Ease of Booking"] : null,
        ...
        waiting: consultedDoctor ? areaRatings["Waiting Time"] : null,

        quality: consultedDoctor
          ? areaRatings["Quality of Consultation"]
          : null,

        staff: consultedDoctor ? areaRatings["Staff Behaviour"] : null,
      ...
      const response = await createPatientFeedbackApi(payload);
      const result = response.data;

      if (!result?.success) {
        const message = result?.message || "Failed to submit patient feedback.";
        setSubmitError(message);
        toast.error(message);
        return;
      }

      const feedbackId = result.data?.patient_feedback_id;

      setFeedbackReference(feedbackId ? `SN-FBK-${feedbackId}` : "SN-FBK-");
      setSubmitted(true);

      toast.success("Your feedback successfully submitted");
    } catch (error: unknown) {
      console.error("PATIENT FEEDBACK API ERROR:", error);
        ...
        <div className="space-y-4 p-5">
                ...
                <div className="mb-4 flex gap-6">
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      onChange={() => handleConsultationChange(true)}
                    />
                    Yes
                  </label>
                  ...
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      name="consultedDoctor"
                      checked={!consultedDoctor}
                      onChange={() => handleConsultationChange(false)}
                    />
                    No
                  </label>
                </div>
          ...
          {submitted && (
            <div className="grid grid-cols-1 items-center gap-4 rounded-lg border border-green-200 bg-green-50 p-4 md:grid-cols-4">
              ...
              <div>
                <p className="text-xs text-gray-500">Feedback Reference No.</p>

                <p className="font-semibold">
                  {feedbackReference || "SN-FBK-"}
                </p>
              </div>
