...
const SpDoctorList = () => {
  ...
  bookingStart.setDate(bookingStart.getDate() + 16);

  const bookingEnd = new Date(bookingStart);
  bookingEnd.setMonth(bookingEnd.getMonth() + 1);

  const bookingStartDate = formatDateToYMD(bookingStart);
  ...
  const handleDateClick = (
    ...
    if (fullDate < bookingStartDate || fullDate > bookingEndDate) {
      return;
    }
                ...
                <div className="grid grid-cols-7">
                  ...
                  {[...Array(daysInMonth)].map((_, i) => {
                    ...
                    const isWithinBookingRange =
                      fullDate >= bookingStartDate &&
                      fullDate <= bookingEndDate;

                    const isBookable = hasSlot && isWithinBookingRange;

                    return (
                      <div
                        key={day}
                        onClick={() =>
                          isBookable && handleDateClick(fullDate, slotInfo)
                        }
                      ...
                      >
                        ...
                        {slotInfo && (
                          <div className={!isWithinBookingRange ? "opacity-50" : ""}>
